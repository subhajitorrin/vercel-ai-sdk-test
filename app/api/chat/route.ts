...
const interviewStage: string = "technical";
const candidateInfo = {
  name: "Subhajit Ghosh",
...
export const POST = async (request: Request) => {
  try {
    ...
    const stages: Stages = {
      introduction: {
        description: `
          Start with friendly, open-ended questions to establish a comfortable atmosphere. 
    ...
    // Generate dynamic prompt based on the stage.
    const prompt = `
      ...
      Respond to the applicant's last message and progress to the next relevant question. Avoid repeating questions already asked.
    ...
    const stream = streamText({
      ...
      prompt,
      system:
        "You are an AI interview agent conducting structured interviews for job applicants."
    });
